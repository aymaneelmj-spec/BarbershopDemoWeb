...
let PHONE = '966000000000';
  ...
  var CURRENCY_TABLE = {
    ...
    LY: { code: 'LYD', ar: '\u062F\u064A\u0646\u0627\u0631',  en: 'LYD', rate: 0.13, decimals: 0 },
    EG: { code: 'EGP', ar: '\u062C\u0646\u064A\u0647',        en: 'EGP', rate: 13,   decimals: 0 },
    MA: { code: 'MAD', ar: '\u062F\u0631\u0647\u0645',        en: 'MAD', rate: 2.6,  decimals: 0 },
    AE: { code: 'AED', ar: '\u062F\u0631\u0647\u0645',        en: 'AED', rate: 0.98, decimals: 0 },
    QA: { code: 'QAR', ar: '\u0631\u064A\u0627\u0644',        en: 'QAR', rate: 0.97, decimals: 0 },
    KW: { code: 'KWD', ar: '\u062F\u064A\u0646\u0627\u0631',  en: 'KWD', rate: 0.082,decimals: 2 },
    BH: { code: 'BHD', ar: '\u062F\u064A\u0646\u0627\u0631',  en: 'BHD', rate: 0.1,  decimals: 2 },
    OM: { code: 'OMR', ar: '\u0631\u064A\u0627\u0644',        en: 'OMR', rate: 0.1,  decimals: 2 },
    JO: { code: 'JOD', ar: '\u062F\u064A\u0646\u0627\u0631',  en: 'JOD', rate: 0.19, decimals: 2 },
    TN: { code: 'TND', ar: '\u062F\u064A\u0646\u0627\u0631',  en: 'TND', rate: 0.82, decimals: 2 },
    DZ: { code: 'DZD', ar: '\u062F\u064A\u0646\u0627\u0631',  en: 'DZD', rate: 36,   decimals: 0 },
  ...
  function convertPrice(rawPrice) {
    if (typeof rawPrice !== 'string' || !rawPrice) return rawPrice;
    var parts = rawPrice.split('-');
    var converted = parts.map(function (p) {
      var n = parseFloat(p);
      if (isNaN(n)) return p;
      var v = n * activeCurrency.rate;
      v = activeCurrency.decimals > 0 ? v.toFixed(activeCurrency.decimals) : Math.round(v).toString();
      return v;
    });
  ...
  function currencyLabel() {
    return currentLang === 'ar' ? activeCurrency.ar : activeCurrency.en;
  }

  function isMultiTierPriceSafe(price) {
  ...
  function refreshAllPriceDisplays() {
    // Service cards: <span class="service-price">120 <span class="currency">ريال</span></span>
    document.querySelectorAll('[data-price]').forEach(function (el) {
      var base = el.getAttribute('data-price');
      if (!base || isMultiTierPriceSafe(base)) return; // tiers shown inside chat, not on the card
      var priceEl = el.querySelector('.service-price');
      if (!priceEl) return;
      priceEl.textContent = convertPrice(base) + ' ';
      var span = document.createElement('span');
      span.className = 'currency';
      span.textContent = currencyLabel();
      priceEl.appendChild(span);
    });

    // chatFlow service objects (used by the chat-bot booking flow)
    Object.keys(chatFlow).forEach(function (catKey) {
      chatFlow[catKey].services.forEach(function (svc) {
        if (!svc._basePrice) svc._basePrice = svc.price; // remember original SAR base once
        svc.price = convertPrice(svc._basePrice);
      });
    });
  }
  ...
  function startServiceBooking(serviceAr, serviceEn, price, platform) {
    ...
    selectedService = { ar: serviceAr, en: serviceEn, price: displayPrice };
    selectedServiceName = currentLang === 'ar' ? serviceAr : serviceEn;
    selectedServicePrice = displayPrice;
    ...
    setTimeout(async function () {
      addBotMsg(
        '\uD83D\uDC88 ' + serviceAr + ' \u2014 ' + displayPrice + ' ' + activeCurrency.ar + '\n\u0645\u0627 \u0647\u0648 \u0627\u0633\u0645\u0643 \u0627\u0644\u0643\u0631\u064A\u0645\u061F',
        '\uD83D\uDC88 ' + serviceEn + ' \u2014 ' + displayPrice + ' ' + activeCurrency.en + '\nWhat is your name?'
      );
      await delay(300);
      addNameInput();
    }, 150);
  ...
  function startTierSelection(serviceAr, serviceEn, price, platform) {
    var tiers = getTiersForService(serviceAr, serviceEn, convertPrice(price));
    ...
    setTimeout(async function () {
      ...
      addOptions(tiers.map(function (tier) {
        return {
          label: (currentLang === 'ar' ? tier.ar : tier.en) + ' \u2014 ' + tier.price + ' ' + (currentLang === 'ar' ? activeCurrency.ar : activeCurrency.en),
          action: async function (lbl) {
  ...
  function buildMsg(lang) {
    ...
    if (lang === 'ar') {
      ...
      msg += '\uD83D\uDC64 *\u0627\u0644\u0627\u0633\u0645:* ' + name + '\n';
      msg += '\uD83D\uDC88 *\u0627\u0644\u062E\u062F\u0645\u0629:* ' + svcAr + '\n';
      msg += '\uD83D\uDCB0 *\u0627\u0644\u0645\u0628\u0644\u063A:* ' + price + ' ' + activeCurrency.ar + '\n';
      msg += '\uD83D\uDCC5 *\u0627\u0644\u0645\u0648\u0639\u062F:* ' + dt + '\n';
      msg += D + '\n';
      ...
      msg += '\uD83D\uDC64 *Name:* ' + name + '\n';
      msg += '\uD83D\uDC88 *Service:* ' + svcEn + '\n';
      msg += '\uD83D\uDCB0 *Amount:* ' + price + ' ' + activeCurrency.en + '\n';
      msg += '\uD83D\uDCC5 *Appointment:* ' + dt + '\n';
      msg += D + '\n';
  ...
  function buildPaymentMsg(lang, methodKey) {
    ...
    var msg = '';
    // NOTE: STC Pay / Mada / the +966 transfer number are Saudi-specific
    // payment rails. They only make sense for activeCountry === 'SA'.
    // If you need this demo to look fully correct for other countries,
    // swap in local rails per country here (e.g. Libya: local bank
    // transfer name; Egypt: InstaPay/Vodafone Cash, etc.)
    if (lang === 'ar') {
      var methodLabel =
  ...
  function setAmountText(id, price) {
    var el = document.getElementById(id);
    if(!el) return;
    var sarLabel = currentLang === 'ar' ? activeCurrency.ar : activeCurrency.en;
